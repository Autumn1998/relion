...

{
    ...
    
    {
      case MRC_MODE_BYTE:
		  ...
		  data_byte[x+y*head->nx]=(unsigned char)pixel_gray[0];
...
//check the mrc file to make sure the size is exact in register with the head
	...
	{
		case MRC_MODE_BYTE:
			if(get_file_size(fin) - 4034 != head->nx*head->ny*head->nz*sizeof(char))
			{
		...
		case MRC_MODE_SHORT:
		case MRC_MODE_USHORT:
			if(get_file_size(fin) - 4034 != head->nx*head->ny*head->nz*sizeof(short))
			{
				printf("Error with Function 'mrc_read_all()'! File size erro!");
			}
			break;
		...
		case MRC_MODE_FLOAT:
			if(get_file_size(fin) - 4034 != head->nx*head->ny*head->nz*sizeof(float))
			{
				printf("Error with Function 'mrc_read_all()'! File size erro!");
			}
			break;
