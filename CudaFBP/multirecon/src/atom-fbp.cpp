...
int Slice_backproj_bilinear2(Pixel pixel, Projection prj, Volume vol, Weight *comp_prj, double *x_coef, double *y_coef, float *prj_real,float *slc_data,int Z_start, int Z_end, int slcN_per,int pixel_num_per)
{
  double s;
  double c;
...

      {
       for (pixel.X=vol.Xstart;pixel.X<vol.Xend;pixel.X++)
        ...
        /* if (pixel.X==-62 && pixel.Y==82 && pixel.Z==-12)
            ...
            {
                slc_data[index]=(float)(s/c);
                if(index>pixel_num_per || index<0)
                {
                	printf("Error");
                }
            }
...
//   printf("head->cmap[0] is %c\n",inhead->cmap[0]);
   ...
   prj.AngN=inhead->nz;
   /*
   x->-5.000000:1.000000:1029.000000
   y->14.000000:1.000000:1010.000000
   z->-33.000000:1.000000:32.000000
   vol.X=1035;
   vol.Y=997;
   vol.Z=66;
   ...
   */
/*
    x->-37.000000:1.000000:1042.000000
    y->-61.000000:1.000000:1042.000000
    z->-33.000000:1.000000:32.000000
   vol.X=1080;
   vol.Y=1104;
   vol.Z=66;
...
/*
   ...
   /* new double
       x->-23.000000:1.000000:1018.000000
       y->0.000000:1.000000:1088.000000
       z->-32.000000:1.000000:25.000000
      vol.X=1042;
      vol.Y=1089;
      vol.Z=58;
      */
...
/*   int filtlength=251;
  ...
  /* int angle;
   ...
   //Slice_backproj_bilinear2(pixel, prj, vol, comp_prj, x_coef, y_coef, prj_real,slc_data,Z_start,Z_end,slcN_per,pixel_num_per); //four-weight
   MPI_Barrier(MPI_COMM_WORLD);
...
/*
   ...
   // mrc_update_head(inf);
        MPI_Offset filesize;
        MPI_File_get_size(fout, &filesize);
        printf("File size : %d \n",filesize);
          }     
