...
int  ATOM_test(char *inf,char *inf2, int myid) //Method ="SIRT" means sirt, Method = "ART" means ART;Method ="SART" means sart
{
	...
	float average = 0;
		...
		if (!myid) {
			...
			for (int i = 0; i < prj.AngN; i++) {
				ncc_arr[i] = NCC(prj_real + (prj.X * prj.Y * i),prj_reproj + (prj.X * prj.Y * i), prj.X, prj.Y);
				average += ncc_arr[i];
				printf("%f   ", ncc_arr[i]);
			}
			printf("\n");
			for (int i = 0; i < prj.AngN; i++) {
					ncc_arr[i] = MAE(prj_real + (prj.X * prj.Y * i),prj_reproj + (prj.X * prj.Y * i), prj.X*prj.Y);
					printf("%f   ", ncc_arr[i]);
			}
			printf("\n");
			for (int i = 0; i < prj.AngN; i++) {
					ncc_arr[i] = scaleRMSRE(prj_real + (prj.X * prj.Y * i),prj_reproj + (prj.X * prj.Y * i), prj.X*prj.Y);
					printf("%f   ", ncc_arr[i]);
			}
			...
			for (int i = 0; i < prj.AngN; i++) {
				...
				ncc_arr[i] = RMSRE(prj_real + (prj.X * prj.Y * i),prj_reproj + (prj.X * prj.Y * i), prj.X*prj.Y);
				printf("%f   ", ncc_arr[i]);
			}
...
int ATOM_subimage(char *inf, char *inf2, char *out1, int myid) //Method ="SIRT" means sirt, Method = "ART" means ART;Method ="SART" means sart
{

	Volume vol;
	Projection prj;
	MPI_File f_out;
	MPI_File_open(MPI_COMM_WORLD, out1, MPI_MODE_CREATE | MPI_MODE_RDWR,
	MPI_INFO_NULL, &f_out);
	MrcHeader *inhead, *inhead2, *outheader;
	inhead = (MrcHeader *) malloc(sizeof(MrcHeader));
	inhead2 = (MrcHeader *) malloc(sizeof(MrcHeader));
	outheader = (MrcHeader *) malloc(sizeof(MrcHeader));
	...
	mrc_init_head(outheader);

	outheader->nx = inhead->nx;
	outheader->ny = inhead->ny;
	outheader->nz = inhead->nz;

	if (myid == 0) {
		mrc_write_head(f_out, outheader);
	}
	...
	if ((input1_data = (float *) malloc(sizeof(float) * line_allnum)) == NULL) {
		printf(
				"Error with Function 'ATOM()'!Can't malloc memery for 'prj_real'!");
		return FALSE;
	}
	memset(input1_data, 0, sizeof(float) * line_allnum);

	if ((input2_data = (float *) malloc(sizeof(float) * line_allnum)) == NULL) {
		printf(
				"Error with Function 'ATOM()'!Can't malloc memery for 'prj_real'!");
		return FALSE;
	}
	memset(input2_data, 0, sizeof(float) * line_allnum);

	if ((res_data = (float *) malloc(sizeof(float) * line_allnum)) == NULL) {
		printf(
				"Error with Function 'ATOM()'!Can't malloc memery for 'prj_real'!");
		return FALSE;
	}
	memset(res_data, 0, sizeof(float) * line_allnum);
	//  printf("hello1\n");
	...
	if (myid == 0) {
		...
		mrc_read_all(fin, inhead, input1_data);
		fclose(fin);
		...
		mrc_read_all(fin2, inhead2, input2_data);
		fclose(fin2);
	...
	if (!myid) {
		...
		for (int i = 0; i < line_allnum; i++) {
			res_data[i] = input1_data[i] - input2_data[i];
		}
		mrc_write_all(f_out, outheader, 0, prj.AngN, res_data);
		mrc_update_head(f_out);
	}
...
int ATOM_meanimage(char *inf)
{
	...
	int line_allnum = vol.X * vol.Y * vol.Z;
	...
	for (int i = 0; i < line_allnum; i++) {
		average += input1_data[i];
	}
	average=average/line_allnum;
	for (int i = 0; i < line_allnum; i++) {
		var += (input1_data[i] - average) * (input1_data[i] - average);
	}
	var = var / line_allnum;
...
int ATOM_readwrite(char *inf,int myid)
{
	...
	MrcHeader *outheader;
	outheader = (MrcHeader *) malloc(sizeof(MrcHeader));
	char *out1="writeres.mrc";
	MPI_File_open(MPI_COMM_WORLD, out1, MPI_MODE_CREATE | MPI_MODE_RDWR, MPI_INFO_NULL, &f_out);
	...
	outheader->nxstart=-259;
	outheader->nystart=-209;
	outheader->nzstart=-32;
	if (myid == 0) {
		mrc_write_head(f_out, outheader);
	}
	...
	if (!myid) {
		...
		mrc_write_all(f_out, outheader, 0,  vol.Z, input1_data);
		mrc_update_head(f_out);
	}
...
//	outheader->nzstart=-32;
	...
	float * input1_data,*output_data;

	int line_allnum = vol.X * vol.Y * vol.Z;
	int line_allnum2 = width * height * outheader->nz;
	if ((input1_data = (float *) malloc(sizeof(float) * line_allnum)) == NULL) {
		printf(
				"Error with Function 'ATOM()'!Can't malloc memery for 'prj_real'!");
		return FALSE;
	...
	if ((output_data = (float *) malloc(sizeof(float) * line_allnum2)) == NULL) {
		printf(
				"Error with Function 'ATOM()'!Can't malloc memery for 'prj_real'!");
		return FALSE;
	}
	memset(output_data, 0, sizeof(float) * line_allnum2);
...
/*
	for(int ix=0;ix<vol.X;ix++)
		for(int iy=0;iy<vol.Y;iy++)
			for(int iz=0;iz<vol.Z;iz++)
			{
				if(ix>=cutxs && ix<cutxs+width && iy>=cutys  && iy<cutys+height )
					output_data[(ix-cutxs)*width+(iy-cutys)*height+iz*vol.Z]=input1_data[ix*vol.X+iy*vol.Y+iz*vol.Z];
			}
*/

	int index=0;
	for(int zi=0;zi<vol.Z;zi++)   //
		for(int xi=0;xi<vol.Y;xi++)
			for(int yi=0;yi<vol.X;yi++)

		{
			if(xi>=373&&xi<(373+width) && yi>=388 &&yi <(width +388))
			{
				output_data[index]=input1_data[xi*vol.X+yi + zi*vol.Y*vol.X];
				index++;
			}
		}
	printf("%d \n",index);
	...
	//for(int i=0;i<vol.X;i++)
	//{
	//	for(int j=0;j<vol.Y;j++)
	//		printf("%f \n",input1_data[1520*vol.X+1466]);//input1_data[i*vol.X+j*vol.Y]
	//}imod

	if (!myid) {

		mrc_write_all(f_out, outheader, 0,  vol.Z, output_data);
		mrc_update_head(f_out);
	}
